...
const SessionsList = () => {
  ...
  const getAllUsers = async () => {
    try {
      ...
      console.log(err);
      toast.error("Something went wrong in getting all sessions || internet issue");
      setLoading(false);
    }
  }
  ...
  // This handleDelete function seems unrelated to the current task and refers to 'users' instead of 'sessions'.
  // I will comment it out or remove it if it's not being used.
  const handleDelete = async () =>{
    // Assuming this is for deleting sessions, it should be updated.
    // const res = await axios.delete(`${BASE_URL}/api/sessions/${sessionId}`);
    // setSessions(sessions.filter(session => session.id !== sessionId));
    const res = await axios.get(`${BASE_URL}/api/users`); // This line is incorrect as it fetches users and not sessions.
    // setUsers(Array.isArray(res.data) ? res.data : []); // This line refers to setUsers which is not defined.
  }
  ...
  return (
    <div>
      ...
      <section className=' pt-10 bg-gray-900'>
         ...
         <div>
            <Heading name="All Sessions" />
            <p className="text-gray-400 text-lg mt-2 text-center">View and manage all scheduled sessions</p>
